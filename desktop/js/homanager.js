 ...
 */
 ...
 $('.eqLogicAttr[data-l1key=configuration][data-l2key=motor]').on('change',function(){
    $('.motor').hide();
    $('.motor_'+$(this).value()).show();
});

 $('.eqLogic').on('click','.listCmd',function(){
    var cmd = {};
    if($(this).attr('data-cmdType') != undefined){
        cmd.type = $(this).attr('data-cmdType');
    }
    if($(this).attr('data-cmdSubType') != undefined){
        cmd.subType = $(this).attr('data-cmdSubType');
    }
